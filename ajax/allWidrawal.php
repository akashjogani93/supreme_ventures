...
if(isset($_POST["submit"]))
{
    ...
    while($row = mysqli_fetch_row($queryRecords) )
    {
        ...
        if($row[7]=='')
        {
            $temp[5]='<input type="file" id="bond" class="form-control file-input" value="Agreement" title = "Choose a video please" style="width:30%;" onchange="bondchange(this)">';
        }else
        {
            $path='./ajax/'.$row[7];
            $temp[5]='<div style="display:flex;"><a href="'.$path.'" target="_blank" class="btn btn-info">Agrement</a><input type="file" id="bond" class="form-control file-input" style="width:30%;" onchange="bondchange(this)"></div>';
        }
