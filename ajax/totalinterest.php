       ...
       <?php if(isset($_POST['full1']))
            {
                    ...
                    $data="";
                    $query="SELECT `invest`.*,`register`.`bank`,`register`.`account`,`register`.`cid`,`register`.`mobile` FROM `invest`,`register` WHERE `invest`.`cid`='$full' AND `invest`.`cid`=`register`.`cid`  ORDER BY `invest`.`id`";
                    ...
                    array_push($a,$total,$amt1,round($totalint),$pmonthtotal,round($totalinterest),$refinvest,round($maininterest),$mainmonth,$bank,$account,$mobile);  
                    echo json_encode($a);
            } 
