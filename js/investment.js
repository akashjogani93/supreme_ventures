$(document).ready(function()
{
    ...
    $("#form").on('submit',(function(e){
        ...
        let log = $.ajax({
            ...
            {
                const data = parseInt(datato.match(/\d+/)[0]);
                if(data==0)
                {
    ...
    //autocomplete referal names
    $(document).on('focus','.referal',handleAuctocomplet);
});

//search button to search data and fetch that
function searchfull()
{
    var cid = $('#full1').val();
    //console.log(full);
    let log = $.ajax({
        ...
        success: function(data) {
            $("#bank11").val(data[0].bank);
            $("#acc11").val(data[0].account);
            $("#mobile11").val(data[0].mobile);
